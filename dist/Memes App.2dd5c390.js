class Controller {
    ...
    init() {
        this.api.getMemes().then((data)=>{
            ...
            this.model.setMemes(memes); //.slice(0,20)
        });
    }
    ...
    handleViewTextTopChange = (text)=>{
        const maxLength = 10;
        if (text.length <= maxLength) this.model.setTextTop(text);
    };
    handleViewTextBottomChange = (text)=>{
        const maxLength = 10;
        if (text.length <= maxLength) this.model.setTextBottom(text);
    };
}

//# sourceMappingURL=Memes App.2dd5c390.js.map
